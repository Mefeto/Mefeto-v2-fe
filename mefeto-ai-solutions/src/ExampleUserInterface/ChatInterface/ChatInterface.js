...
export default function ChatInterface() {
    ...
    return (
        <div style={{ width: "700px", margin: "0 auto" }}>
            <h1 style={{ textAlign: "center" }}>ChatGPT(with function call) Interface</h1>
            {chatLog.map(
                log => (
                    <div className={log.role}>
                        ...
                        {log.function_call !== undefined &&
                            <p style={{ display: "inline" }}>call function {log.function_call.name} ({log.function_call.arguments})</p>}
                    </div>
                )
            )}
            <div className='ChatInput'>
                ...
                ></input>
                <button className="submitChat" onClick={onClick}>✨</button>
            </div>
        </div>
    );
}