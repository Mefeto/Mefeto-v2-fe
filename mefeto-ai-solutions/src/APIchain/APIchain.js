...
async function APIchain(query) {
    const totalResponse = await fetchFromAPI(
        ...
        data => {
            ...
            return [data.dlytrmSearch.일상용어].flat().map(
                word => word.용어간관계링크._cdata.match(/(?<=(MST=))\d+/g)[0]
            );
        }
    ).then(
        async MSTs => {
        ...
        async data => {
            ...
            return await Promise.all(
                data.flat().map(
                    data1 => data1.dlytrmRltService.일상용어.연계용어
                ).flat().map(
        ...
        data => {
            ...
            return data.map(
                ...
                law => {
                    ...
                    try {
                        return {
                            종합법령명: `${law.법령명} 제${law.조번호}조` + (law.조가지번호 === 0 ? "" : `의${law.조가지번호}`) + law.조문내용.match(/\((.*?)\)/)[0],
                            조문내용: (" " + law.조문내용.substring((law.조문내용).indexOf('①') - 1)).split('\n').filter(str => str.trim() !== '')
                        };
                    } catch (e) {
                        return null;
                    }
